package api

import (
	"net/http/httptest"
	"testing"

	"github.com/gofiber/fiber/v2"

	"github.com/knightsofeternity/kfire-server/internal/config"
	"github.com/knightsofeternity/kfire-server/internal/connectors/riot"
)

// TestRiotRoutesMountedAndProtected proves the five Riot routes are actually
// wired: the start route requires a bearer token while the callback, reached
// by the browser coming back from Riot, is public.
//
// The package has no fixture that mounts the full api.Register (it needs a
// live database for the store), so this test mounts only the Riot routes
// directly on the handlers struct, the same handler methods Register wires,
// with a disabled Riot connector (no client id/secret). That is enough to
// exercise requireAuth's gate and the callback's own "connector disabled"
// early return, both of which happen before any store access.
func TestRiotRoutesMountedAndProtected(t *testing.T) {
	h := &handlers{
		cfg:  &config.Config{JWTSecret: "test-secret"},
		riot: riot.New("", "", ""), // disabled: no client id configured
	}

	app := fiber.New()
	v1 := app.Group("/api/v1")
	v1.Get("/connect/riot", h.requireAuth, h.connectRiotStart)
	v1.Get("/connect/riot/callback", h.connectRiotCallback)
	v1.Get("/connect/riot/region", h.requireAuth, h.riotRegion)
	v1.Patch("/connect/riot/region", h.requireAuth, h.updateRiotRegion)
	v1.Delete("/connect/riot", h.requireAuth, h.disconnectRiot)

	t.Run("start requires auth", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/api/v1/connect/riot", nil)
		resp, err := app.Test(req)
		if err != nil {
			t.Fatalf("request failed: %v", err)
		}
		if resp.StatusCode != fiber.StatusUnauthorized {
			t.Fatalf("start without token: got %d, want 401", resp.StatusCode)
		}
	})

	t.Run("callback is public", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/api/v1/connect/riot/callback", nil)
		resp, err := app.Test(req)
		if err != nil {
			t.Fatalf("request failed: %v", err)
		}
		if resp.StatusCode == fiber.StatusUnauthorized {
			t.Fatalf("callback without token: got 401, want a public response")
		}
	})

	t.Run("region and disconnect require auth", func(t *testing.T) {
		cases := []struct {
			method, path string
		}{
			{"GET", "/api/v1/connect/riot/region"},
			{"PATCH", "/api/v1/connect/riot/region"},
			{"DELETE", "/api/v1/connect/riot"},
		}
		for _, tc := range cases {
			req := httptest.NewRequest(tc.method, tc.path, nil)
			resp, err := app.Test(req)
			if err != nil {
				t.Fatalf("%s %s: request failed: %v", tc.method, tc.path, err)
			}
			if resp.StatusCode != fiber.StatusUnauthorized {
				t.Errorf("%s %s without token: got %d, want 401", tc.method, tc.path, resp.StatusCode)
			}
		}
	})
}
